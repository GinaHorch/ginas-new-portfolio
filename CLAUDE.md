...
- **`src/components/skills/skillsData.ts`** — the `/skills` data: capability groups and per-skill *evidence levels* (`production` / `practical` / `working` / `developing`). Deliberately not numeric scores. `SkillGroupList` renders the tags; `OverviewSkillsChart` renders the single summary chart. Group titles must match `skills.technical.skills[].title` in `content.js`. Levels that rest on study rather than delivered work trace to an evidence record — `.claude/context/diploma-practical-labs.md` and `.claude/context/soc-blue-team-training.md` — so a level can be checked rather than asserted; add one before adding that kind of skill.
